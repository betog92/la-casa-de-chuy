...
import { fetchDefaultAuthenticatedPath } from "@/lib/auth/default-authenticated-path";
    ...
    try {
      ...
      if (result.success) {
        if (onSuccess) {
          ...
          router.push(await fetchDefaultAuthenticatedPath());
        }
      } else {
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            {...register("password")}
            className="w-full px-4 py-3 pr-12 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-[#103948] focus:border-transparent outline-none transition-all"
            placeholder="••••••••"
          />
          <button
            type="button"
